    ...
    >
      ...
      <div className="block md:hidden relative z-10 py-12 px-6 mt-16 pb-40">
        <div className="bg-white/40 backdrop-blur-md rounded-3xl shadow-xl p-6 border border-white/60">
          ...
          <a href={config.hotel.mapUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 text-lg text-slate-900 mb-6 hover:underline" style={textGlow}>
            <MapPin size={18} className="text-purple-900" />
            {config.hotel.addressShort}
          </a>

          <div className="space-y-4 mb-8 text-center">
            <p className="text-lg text-slate-900 " style={textGlow}>
               A room block is available for the night of {config.dates.hotelNightDate}.
            </p>
            <p className="text-lg text-slate-900" style={textGlow}>
              Book by <span className="text-purple-900">{config.dates.hotelBookingDeadline}</span> for the group rate.
            </p>
          </div>

          <a href={config.hotel.bookingUrl} target="_blank" rel="noopener noreferrer" className="block w-full bg-purple-900/90 backdrop-blur-sm text-white text-center py-4 rounded-xl font-bold text-lg shadow-lg active:scale-95 transition-transform">
            BOOK ROOM
          </a>
        </div>
      </div>
      ...
      <div className="hidden md:flex relative z-10 min-h-screen flex-col justify-center items-center py-12">
        <div className="w-full max-w-xl bg-white/20 backdrop-blur-sm rounded-3xl shadow-2xl p-12 border border-white/40">
          ...
          <h2 className="text-4xl text-center mb-2 text-slate-900" style={textGlow}>{config.hotel.name}</h2>
           <a
    href={config.hotel.mapUrl}
    target="_blank"
  ...
  >
    {config.hotel.address}
          ...
          <div className="text-center space-y-4 mb-10">
            <p className="text-xl text-slate-900 f" style={textGlow}>A room block is available for the night of {config.dates.hotelNightDate}.</p>
            <p className="text-xl text-slate-900 " style={textGlow}>Be sure to book by <span className="text-purple-900">{config.dates.hotelBookingDeadline}</span> to get the group rate.</p>
          </div>
          <div className="text-center">
            <a href={config.hotel.bookingUrl} target="_blank" rel="noopener noreferrer" className="inline-block bg-purple-900/90 text-white px-10 py-4 rounded-full text-xl font-bold hover:bg-purple-800 transition-all shadow-lg">BOOK YOUR ROOM</a>
          </div>
        </div>
      </div>
    </main>
